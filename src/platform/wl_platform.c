...
bool platform_init_wayland(struct platform *platform) {
	...
	*(void **)(&_wl_library.display_dispatch) = dlsym(handle, "wl_display_dispatch");
	...
	platform->internal->framebuffer = wl_get_framebuffer;
...
Framebuffer wl_get_framebuffer(struct platform *platform) {
	return (Framebuffer){
		.pixels = NULL,
		.width = platform->physical_width,
		.height = platform->physical_height,
...
struct wl_buffer *create_shm_buffer(struct platform *platform) {
	const int buffer_stride = platform->logical_width * 4;
	const int buffer_size = platform->logical_width * platform->logical_height * 4;

	const int fd = create_anonymous_file(buffer_size);
	if (fd < 0) {
		LOG_ERROR("Wayland: Failed to create buffer file of size %d: %s", strerror(errno));
		return NULL;
	}

	void *data = mmap(NULL, buffer_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
	if (data == MAP_FAILED) {
	...
	struct wl_shm_pool *pool = wl_shm_create_pool(platform->internal->wl.shm, fd, buffer_size);
